...
export const api = {
  ...
  getCollection: (slug: string) =>
    request<{ collection: CollectionItem; products: Product[] }>(`/api/collections/${slug}`),
  ...
  getSearchSuggestions: (q: string) =>
    request<SearchSuggestion[]>(`/api/search/suggest?q=${encodeURIComponent(q)}`),

  getProductReviews: (slug: string) => request<Review[]>(`/api/products/${slug}/reviews`),
  createProductReview: (slug: string, payload: { rating: number; text: string }) =>
  ...
  toggleFavorite: (productId: string) =>
    request<{ ok: boolean; active: boolean; favoriteIds: string[] }>(
  ...
  forgotPassword: (payload: Record<string, string>) =>
  ...
  updateProfile: (payload: Partial<UserProfile>) =>
    request<UserProfile>("/api/profile/me", {
      method: "PUT",
      body: JSON.stringify(payload),
    }),
  ...
  createOrder: (payload: CheckoutPayload) =>
    request<PaymentSession>("/api/checkout/create", {
      method: "POST",
      body: JSON.stringify(payload),
    }),
  ...
  getAdminOverview: () =>
    request<{
      ...
      orders: number;
      users: number;
      billzEnabled: boolean;
    }>("/api/admin/overview"),

  getAdminCategories: () => request<Category[]>("/api/admin/categories"),
  createAdminCategory: (payload: Partial<Category> & { name?: string; description?: string }) =>
    request<Category>("/api/admin/categories", {
      method: "POST",
      body: JSON.stringify(payload),
    }),
  updateAdminCategory: (id: string, payload: Record<string, unknown>) =>
  ...
  updateAdminProduct: (id: string, payload: Partial<Product>) =>
  ...
  createAdminProduct: (payload: Partial<Product>) =>
  ...
  syncBillz: () =>
    request<{ message: string; imported: number }>("/api/admin/billz/sync", {
      method: "POST",
    }),

  uploadProductImage: (id: string, payload: { fileName: string; dataUrl: string }) =>
    request<Product>(`/api/admin/products/${id}/image`, {
      method: "POST",
      body: JSON.stringify(payload),
    }),
};